...
def main():
    ...
    # 默认非流式 ，使用2个模型，收到消息的时候指定模型名称就会调用该模型
    ag1 = BaseAgent(
        name="wltest1",
        llm_config=llm_config,
        max_num_auto_reply = 5,
    )
    ...
    # 将函数转换为工具,注册函数
    #  给 wltest1 这个agent注册工具 get_current_weather
    ...
    print("-----------x:",x)
    # 指定 api_type 为 openai,默认调用异步函数。
    x = ag1(x, use_tools=True, api_type = "openai")
    print("--ag1-----------------")
    print(x)
    print("--ag1-----------------")
    #'''
