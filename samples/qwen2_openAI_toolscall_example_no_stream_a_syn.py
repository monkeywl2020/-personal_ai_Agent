...
def main():
    ...
    x = ag1(x, use_tools=True, api_type = "qwen-openai")
    print("--ag1-----------------")
    print(x)
    print("--ag1-----------------")
    ...
    # 这个是用户agent，注册了执行函数
    #------------------------------------
    ag2.register_tool_for_llm(query_scale_category)
    ag2.register_tool_for_llm(query_scale_list)
    ag2.register_tool_for_llm(create_pa_test_task)
    x = ag2(x)
    print("--ag2---------x:",x)
   
   #------------------------------------
   #  ******  单function call 函数执行后的调用 ****
   #------------------------------------
    x = ag1(x, use_tools=True, api_type = "qwen-openai")
    print("--ag1-----------------")
    print(x)
