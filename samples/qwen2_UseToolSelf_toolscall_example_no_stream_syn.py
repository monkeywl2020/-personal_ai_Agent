...
def main():
    ...
    #--------------------------------------------------
    def process_msg_to_client(text):
        print("stream_to_client--- Start of Chunk -ccc------------------",flush=True)
        print(text,flush=True)
        print("stream_to_client--- End of Chunk -ccc------------------",flush=True)

    x = Msg(name="user",role="user", content=content)
    print("-----------x:",x)
    # 指定 api_type 为 openai
    x = ag1.generate_response(x, use_tools=True, api_type = "qwen", stream_callback = process_msg_to_client)
    print("--ag1-----------------")
    print(x)
    print("--ag1-----------------")
