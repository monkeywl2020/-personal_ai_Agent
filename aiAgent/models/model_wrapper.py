...
class ModelChatWrapper:
    ...
    def _register_model_client(self, config: Dict[str, Any], tool_manager: Optional[ToolManager] = None) -> None:
        ...
        if api_type is not None and api_type == "llama":
            client = LlamaChatWarpperClient(**config, tool_manager = tool_manager) # llama的客户端
            self._clients.append(client) #将client按照配置添加进来

        elif api_type is not None and api_type == "qwen-openai":
            LOG_INFO("_register_model_client-----------Qwen2_5_OpenAiChatWarpperClient")
            client = Qwen2_5_OpenAiChatWarpperClient(**config, tool_manager = tool_manager) # Qwen的客户端
            self._clients.append(client)

        elif api_type is not None and api_type == "qwen":
            LOG_INFO("_register_model_client-----------QwenChatWarpperClient")
            client = QwenChatWarpperClient(**config, tool_manager = tool_manager) # Qwen的客户端
            self._clients.append(client)
    ...
    def _get_client(self, api_type: Optional[str]) -> ModelClient:
        ...
        if self._clients is not None:
            if api_type is not None:
                return next((client for client in self._clients if api_type == client.api_type), self._default_client)
            else:
