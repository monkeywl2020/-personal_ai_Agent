...
#-----------------------------------
class LlamaChatWarpperClient(ModelClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        LOG_INFO("LlamaChatWarpperClient::__init__-----------")
    ...
    def format(self, *args: Union[Msg, Sequence[Msg]]) -> List[dict]:
        ...
        #---------------------------------------------------
        for arg in args:
            LOG_INFO("LlamaChatWarpperClient::format:=============== arg in args:",arg)
            ...
            if isinstance(arg, Msg): # 如果是 Msg
                if arg.url is not None:# 如果有 url
                    ...
                    if arg.role != "tool":
                        ...
                        #---------------------------------------------------
                        if arg.get("message"):
                            LOG_INFO("LlamaChatWarpperClient::format:=============== message in Msg:",arg.get("message"))
                            messages.append(arg.get("message"))

                        # 下面是工具调用结果的消息
                        elif arg.get("tool_responses"):
                            LOG_INFO("LlamaChatWarpperClient::format:=============== tool_responses in Msg:",arg.get("tool_responses"))
                            ...
                            messages.append(arg)
                LOG_INFO("LlamaChatWarpperClient::format:=============== arg to message:",messages)
        ...
        LOG_INFO("LlamaChatWarpperClient::format:=============== end! final messages:",messages)
        return messages
    
    #---------------------------------------------------------
    #  解析 转换 function call的响应格式的函数  
    #  将llama的function call 格式转成openAI的格式，也就是 tool_calls 字段内容
    #---------------------------------------------------------
    def parse_function_call(content):
        """Parse a single function call from content and create a tool call."""
        # Remove the <|python_tag|> if present
        if content.startswith('<|python_tag|>'):
            content = content.replace('<|python_tag|>', '', 1).strip()

        # 解析成json格式
        function_data = json.loads(content)

        # 新建openAI的tool call格式, arguments要转成str格式
        tool_call = {
            "function": {
                "arguments": json.dumps(function_data['parameters']),
    ...
    def format_function_call_rsp(self, response: dict) -> List[dict]:
        #---------------------------------------------------
        # 1： 跟openAI返回的 fucntion call 格式有些不一样
        #     message 中的内容不一样，与普通消息的content内容不一样
        #     需要转成 openAI的格式统一在外部处理
        #---------------------------------------------------
        '''
            下面这个是 llama 的 function call 格式,
            单个function call的格式中content 以 "<|python_tag|>" 开头，多个function call 以 "{" 开头            
            {
                "id": "chat-e79e7eedbb9f4542a2130b45a2e9feea",
                "choices": [
                    {
                        "finish_reason": "stop",
                        "index": 0,
                        "logprobs": null,
                        "message": {
                            "content": "{\"name\": \"get_current_weather\", \"parameters\": {\"location\": \"\\u4e4c\\u91ce\", \"unit\": \"c\"}}; {\"name\": \"get_current_weather\", \"parameters\": {\"location\": \"\\u5317\\u4eac\", \"unit\": \"c\"}}; {\"name\": \"get_current_weather\", \"parameters\": {\"location\": \"\\u6df1\\u5733\", \"unit\": \"c\"}}",
                            "refusal": null,
                            "role": "assistant",
                            "function_call": null,
                            "tool_calls": []
        ...
        # 获取 content 中的内容，这内容是function call的内容
        content = response["choices"][0]["message"]["content"]

        # 如果是单个function call,以 "<|python_tag|>" 开头
        if content.startswith('<|python_tag|>'):
            # Handle a single function call
            tool_calls = [self.parse_function_call(content)]
        else:
            # Handle multiple function calls
            weather_requests = [req.strip() for req in content.split(";")] # 以分号分隔,去掉多余的空格
            tool_calls = [self.parse_function_call(req) for req in weather_requests]

        # 实际只需要message内容，其中包含 原始的raw内容，和tool call的内容
        message = {
    ...
    def _generate_response(
        ...
        LOG_INFO("LlamaChatWarpperClient::_generate_response:---------11-- kwargs", kwargs)
        ...
        # 3： 调用模型接口 同步
        #-------------------------------------------------------------
        LOG_INFO("LlamaChatWarpperClient::_generate_response:-------cccc---- kwargs:\n",kwargs)
        response = self.client.chat.completions.create(**kwargs)
        LOG_INFO("LlamaChatWarpperClient::_generate_response:-------cccc---- response:\n",response)
        ...
        if stream:
            ...
            try:
                ...
                LOG_INFO("LlamaChatWarpperClient::_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("LlamaChatWarpperClient::_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("LlamaChatWarpperClient::_generate_response: -------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("LlamaChatWarpperClient::_generate_response: -------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("LlamaChatWarpperClient::_generate_response: -------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"LlamaChatWarpperClient::_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
            ...
            LOG_INFO("LlamaChatWarpperClient::_generate_response:-------ccc222---- response:\n",response)

            #content = response["choices"][0]["message"].get("content")
            #return None
            
            if self._verify_text_content_in_llama_message_response(response):
                #---------------------------------------------------
            ...
            elif self._verify_function_call_in_llama_message_response(response):
                LOG_INFO("LlamaChatWarpperClient::_generate_response:----------- a function call response:",response) 
    ...
    async def _a_generate_response(
        ...
        # 3： 调用模型接口 异步
        #-------------------------------------------------------------
        LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------cccc---- kwargs:",kwargs)
        response = self.client.chat.completions.create(**kwargs)
        LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------cccc---- response:",response)
        ...
        if stream:
            ...
            try:
                ...
                first_chunk = first_chunk.model_dump()
                firs_role = first_chunk["choices"][0]["delta"]["role"]
                LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------dddd-1-stream-- first_chunk:\n",first_chunk)
                ...
                LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------dddd-2-stream-- first_chunk:\n",first_chunk)
            except StopIteration:
                raise RuntimeError("Empty response received from OpenAI API.")
            ...
            if self._verify_function_call_in_openai_stream_message_response(first_chunk):
                ...
                LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------dddd--stream-- frist function_call_message:\n",function_call_message)
                for chunk in response:
                    chunk = chunk.model_dump()
                    LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------dddd--stream-- chunk:\n",chunk)
                ...
                LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------dddd--stream-- function_call_message:\n",function_call_message)
                ...
                def generator() -> Generator[str, None, None]:
                    ...
                    LOG_INFO(f"OpenAiChatWarpperClient::_a_generate_response:------------5555555555------------last_chunk text: {text}", flush=True)  # 增加这一行
                    #---------------------------------------------------
                    #  调用回调函数，保存历史消息
                    #---------------------------------------------------
                    abc = Msg(name="assistant",role="assistant",content=text)
            ...
            response = response.model_dump()
            LOG_INFO("OpenAiChatWarpperClient::_a_generate_response:-------cccc222---- response:model_dump():",response)
    ...
    #=======================================================
    def _verify_function_call_in_llama_message_response(self,response: dict) -> bool:
        ...
        # content如果不是 '<|python_tag|>' 并且也不是 '{' 开头
        content = response["choices"][0]["message"].get("content")
        if not (content.startswith('<|python_tag|>') or content.startswith('{')):
    ...
    #=======================================================
    def _verify_text_content_in_llama_message_response(self,response: dict) -> bool:
        ...
        # content不能以 '<|python_tag|>' 开头
        if response["choices"][0]["message"].get("content").startswith('<|python_tag|>'):
            LOG_INFO("=_verify_text_content_in_openAI_message_response========4======False")
            return False
        
        # content不能以 '{' 开头
        if response["choices"][0]["message"].get("content").startswith('{'):
    ...
    #=======================================================
    def _verify_null_msg_in_llama_stream_response(self,response: dict) -> bool:
    ...
    #   检查 是否是function call的应答 在 qwen的响应消息里面
    #=======================================================
    def _verify_function_call_in_llama_stream_message_response(self,response: dict) -> bool:
    ...
    #==========================================================
    def _verify_text_content_in_llama_stream_message_response(self,response: dict) -> bool:
